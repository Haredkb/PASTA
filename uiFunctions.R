    ...
    tabsetPanel(id = ns("nwis_calc"), #type = "tabs",
                tabPanel("Input: Available Stream Sites",
                         sidebarPanel(
                           ...
                           checkboxInput(ns("bfi"), "Calculate baseflow index (BFI) when discharge data are available?", value = TRUE),
                ...
                tabPanel("Results Plots",
                         ...
                         fluidRow(
                           ...
                           tags$hr(),
                           h4("Baseflow Index vs Amplitude Ratio"),
                           plotlyOutput(ns("plot_BFI_AR")))
                         #DT::dataTableOutput("user_dataair")
                )
  ),#end page1
  )
...
envCanUI <- function(id, label = "envCanada automated") {
  ...
  tagList(
    tabsetPanel(id = ns("envC_calc"), type = "tabs",
                tabPanel("Input: Available Stream Sites",
                         sidebarPanel(
                           strong("STEP 1"),
                           strong("Choose station(s) or use map extent"),

                           selectInput(ns("station"), "Stations",
                                       choices = sort(unique(envCan_stations$STATION_NO)),
                                       multiple = TRUE),

                           p("OR Use Map Extent Coordinates (overrides station input)"),
                           checkboxInput(ns("mapextent"), "Use map extent instead of station list?"),
                           verbatimTextOutput(ns("AOI")),

                           actionButton(inputId = ns("getData"), label = "Get Data"),
                           p("Can take a few minutes, especially for multiple stations"),
                           hr(),

                           strong("STEP 2"),
                           p("Select sites from the adjacent table, then run metrics"),

                           radioButtons(ns("yr_type"),
                                        "Conduct Annual Signal Analysis via Water Year (October Start - recommended) or Calendar Year (January Start)?",
                                        choices = c("Calendar" = "calendar",
                                                    "Water Year" = "water"),
                                        selected = "water"),

                           actionButton(inputId = ns("gobutton"),
                                        label = "Calculate Thermal Metrics",
                                        style = "padding:20px; font-size: 22px; color: #fff; background-color: #FF0000; border-color: #2e6da4")
                         ),
                ...
                tabPanel(title = "Results: Metric Tables",
                         ...
                         fluidRow(
                           h4("Metric Data Table"),
                           h5("Grey columns are associated with paired air and stream annual signal calculations"),
                           p("Amp_Ratio is Amplitude Ratio, unitless"),
                           p("PhaseLag_d is Phase Lag, days"),
                           p("Mean_ratio is ratio of average water temperature divided by average air temperature"),
                           h5("Blue columns are associated with air and stream temperature linear regression"),
                           p("TS__Slope is the slope of the linear relationship between air and water temperature"),
                           p("AdjRsqr is the R2 of the linear fit"),
                           p("YInt is the y intercept of the linear relationship"),
                           h5("Please review the literature citations from the Information tab to interpret outputs"),
